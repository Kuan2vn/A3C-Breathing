...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha,
            fc1_dims=128, fc2_dims=256, fc3_dims=128, fc4_dims=64, fc5_dims=32, chkpt_dir='tmp/ppo'):
        ...
        self.actor = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(fc4_dims, fc5_dims),
                nn.ReLU(),
                nn.Linear(fc5_dims, n_actions),
                nn.Softmax(dim=-1)
        )
    ...
    def freshload(self, file_name='fresh/actor.pth'):
        file_name = os.path.join(self.chkpt_dir, file_name)
        self.load_state_dict(T.load(file_name))
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=128, fc2_dims=256, fc3_dims=128, fc4_dims=64, fc5_dims=32,
            chkpt_dir='tmp/ppo'):
    ...
    def freshload(self, file_name='fresh/critic.pth'):
        file_name = os.path.join(self.chkpt_dir, file_name)
        self.load_state_dict(T.load(file_name))
...
class Agent:
    ...
    def fresh_models(self):
        ...
        self.actor.freshload()
        self.critic.freshload()
